 ...
 *  on 15-May-16.
 */
class ParameterListVisitor(val typeProvider: TypeProvider, val expressionVisitor: ExpressionVisitor) : ElaginBaseVisitor<List<Parameter>>() {
    override fun visitFormalParameters(ctx: ElaginParser.FormalParametersContext): List<Parameter> {
        ...
        return formalParameterList.formalParameter().map { it.accept(ParameterVisitor(typeProvider, expressionVisitor)) }
    }
}